...
- [ ] Mở http://127.0.0.1:8000/; trạng thái ba kho đều up.
...
## Áp dụng gói (trước các lệnh build ở trên)

Giải nén ZIP ra ngoài repository, ví dụ `D:\rdw-day3-update`.
Trong PowerShell tại `D:\Document Workspace`:

```powershell
python D:\rdw-day3-update\apply_day3.py --project . --check
if ($LASTEXITCODE -ne 0) { throw 'Source differs. Stop; do not force-copy.' }
python D:\rdw-day3-update\apply_day3.py --project . --apply
if ($LASTEXITCODE -ne 0) { throw 'Apply failed.' }
```

Installer đối chiếu hash nội dung với source Ngày 2 sau refactor; chấp nhận khác CRLF/BOM.
Nếu có thay đổi khác, dừng trước khi ghi. Gửi đúng file được báo khác để điều chỉnh gói.
Không sửa manifest để bỏ qua kiểm tra; không copy đè thủ công toàn bộ project.
Installer backup source vào thư mục cạnh repository và in lệnh rollback chính xác.
Không đọc `.env`, không tự chạy Docker, không tự commit, không ghi/xóa dữ liệu DB.

Rollback chỉ hoàn tác source. Trước khi rollback, hoàn tất thử lại mọi tài liệu `deleting` bằng phiên bản Ngày 3 nếu có.
Sau đó chạy lệnh rollback đã in và rebuild web.
Constraint mở rộng trong DB được giữ lại, tương thích trạng thái Ngày 2; không chạy lệnh thu hẹp constraint/xóa volume.
Rollback source không khôi phục tài liệu đã xóa bằng UI/API.
